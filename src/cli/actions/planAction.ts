 ...
 * conversation whose directory Lantern never recorded, a session id with a
 * quote in it — are settled in tests instead of by spawning processes.
 */
export const planAction = (request: ActionRequest): ActionPlan => {
  ...
  if (request.action === "print") {
    return { kind: "print", text: buildResumeCommand(request.sessionId, request.executable), cwd };
  }

  return {
    kind: "handoff",
    // argv, not a shell line: nothing here needs quoting, and quoting it would
    // make the id part of the argument.
    binary:
