...
export const conversationTopicsQuery = {
  queryKey: ["conversations", "topics"],
  queryFn: async () => {
    const response = await honoClient.api.conversations.topics.$get();

    if (!response.ok) {
      throw new Error(`Failed to fetch conversation topics: ${response.statusText}`);
    }
...
export const conversationTopicsPendingQuery = {
  queryKey: ["conversations", "topics", "pending"],
  queryFn: async () => {
    const response = await honoClient.api.conversations.topics.pending.$get();

    if (!response.ok) {
      throw new Error(`Failed to fetch pending topic count: ${response.statusText}`);
    }
...
export const conversationListQuery = (options: { query: string; limit: number; topic?: string }) =>
  ({
    queryKey: ["conversations", options.query, options.limit, options.topic ?? null],
    queryFn: async () => {
      const response = await honoClient.api.conversations.$get({
        query: {
          query: options.query,
