...
const claudeCodeRoutes = Effect.gen(function* () {
  ...
  const billingModeService = yield* BillingModeService;
  ...
  return (
    new Hono<HonoContext>()
      ...
      /**
       * How Claude Code is billed on this machine, so the UI does not have to ask.
       * Carries the mode and the plan name only - never a credential.
       */
      .get("/billing-mode", async (c) => {
        const response = await effectToResponse(
          c,
          billingModeService.detect.pipe(
            Effect.map((billing) => ({ status: 200 as const, response: { billing } })),
